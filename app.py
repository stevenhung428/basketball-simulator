...
# 籃框位置
x_target, y_target, z_target = 4.5, 3.05, 0.0
...
def simulate_once(theta_rad, v_input):
    ...
    while y >= 0 and x <= 10:
        ...
        y += vy * dt
        z += vz * dt
        if abs(x - x_target) < 0.3 and abs(y - y_target) < 0.3 and abs(z - z_target) < 0.3:
...
try:
    if mode == "🎥 拋物線動畫":
        ...
        traj_x, traj_y, traj_z, _ = simulate_once(theta_rad, v)
        frames = []
        ...
        fig.add_trace(go.Scatter(x=[x_target], y=[y_target], mode='markers+text',
        ...
        if not all_trajectories:
            ...
            fig.add_trace(go.Scatter3d(
                x=[x_target], y=[z_target], z=[y_target],
                mode='markers+text',
